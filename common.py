...
def _fps_pause(frame_time, start_time):
    diff = time.time() - start_time
    sleep(0 if diff >= frame_time else frame_time - diff)

def main_loop(title, calculate_data_func):
    canvas, label, string_var = _init_window(title)
    while True:
        start_time = time.time()
        data, description, frame_time, done = calculate_data_func()
        _redraw_canvas(canvas, data)
        _redraw_label(label, string_var, description)
        _fps_pause(frame_time, start_time)
        if done:
            break
    mainloop()
